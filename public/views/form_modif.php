...
if (isset($_GET["action"]) && !empty($_GET["action"])) {   // Si une action est spécifiée dans l'URL
    switch ($_GET["action"]) {
        ...
        WHERE accueil.id_accueil = 1";
            ...
            if ($result) {
                $msg_accueil = $result["msg_accueil"];
                $paragraphe_un = $result["paragraphe_un"];
                $paragraphe_deux = $result["paragraphe_deux"];
                $paragraphe_trois = $result["paragraphe_trois"];
                $paragraphe_quatre = $result["paragraphe_quatre"];
            } else {
                echo "0 résultats";
            }
?>
